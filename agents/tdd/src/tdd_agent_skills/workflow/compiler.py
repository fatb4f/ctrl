...
def _repository_root(plan_path: Path) -> Path:
    for parent in (plan_path.parent, *plan_path.parents):
        if (parent / "cue.mod" / "module.cue").is_file():
            return parent
    raise WorkflowCompileError(f"could not find CUE module root above {plan_path}")
    ...
    *,
    ...
    try:
        ...
        for block in extracted.blocks:
            record, record_digest = export_block(repository_root, block)
        ...
        fixture_specs = _sidecar(repository_root, fixture_specs_path.resolve(), "fixtureSpecs")
        probes = _sidecar(repository_root, probes_path.resolve(), "probeSpecs")
        realizations = _sidecar(
            repository_root, realization_specs_path.resolve(), "realizationSpecs"
        )
        fixture_manifests = [
            derive_fixture_manifest(
                repository_root,
                fixture,
                fixture_data_root=fixture_specs_path.resolve().parent / "data",
            )
