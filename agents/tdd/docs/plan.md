...
class CaptureIntegrity(FrozenTransport):
    complete: bool
    truncated: bool
    event_loss_detected: bool
    serialization_failures: NonNegativeInt
    instrumentation_error_code: NonEmptyString | None
...
class ProbeObservation(FrozenTransport):
    ...
    observation_schema_id: NonEmptyString
    observed_artifact_digest: ArtifactDigest
    capture_status: Literal["complete", "truncated", "failed", "timed-out"]
...
`ProbeObservation` is a provenance envelope over a raw typed observation artifact. It never authors
`supports`, `refutes`, `qualified`, or another semantic result. The pure evaluation kernel applies
the admitted validators and expected outcome to a complete observation artifact and derives the
`EvaluationResult`. A failed, timed-out, truncated, lossy, or serialization-incomplete capture is
structurally valid diagnostic evidence but cannot satisfy a promotion-grade evidence role.

## CPython runtime evidence and control-policy evaluation

CPython runtime objects never cross the evidence boundary. Provider-specific collectors for
`sys.monitoring`, traceback, frame snapshots, `tracemalloc`, and `faulthandler` project runtime state
into generated, closed Pydantic transports before any artifact is admitted. The contract direction
is one way:

```text
local CUE runtime-evidence contract
  → exported JSON Schema
  → generated frozen Pydantic transports
  → bounded provider normalization
  → canonical JSON artifact
  → CUE validation and pure-kernel evaluation
```

All wire fields use the contract's snake-case names. Generated transports forbid extra fields and
use discriminated unions for observation and value kinds. Runtime value projection is depth-,
item-, string-, byte-, and total-document-bounded. It rejects non-finite floats, represents exact or
out-of-I-JSON-range numbers as tagged strings, preserves non-string mapping keys as typed key/value
entries, and treats object identities as capture-local only. Source coordinates bind a
repository-relative path, source digest, line, end position, explicit column unit, code identity,
and bytecode offset when applicable.

Event order is explicit across process, interpreter, thread, and optional task coordinates. A
capture declares whether its sequence is global or per stream. The integrity contract enforces:

```text
complete  → no truncation, event loss, serialization failure, or instrumentation error
truncated → incomplete and truncated
failed    → incomplete with a structured adapter failure
timed-out → incomplete with a timeout observation
event loss or serialization failure → incomplete
```

The runtime content document does not contain its own digest. Canonical content bytes are stored as
an artifact; `ProbeObservation.observed_artifact_digest` and `ArtifactProduction` bind those bytes
to the rollout event. Timestamps are normalized to one UTC RFC 3339 representation before CUE uses
parsed time values to enforce ordering. Omitted optional fields and explicit `null` have distinct,
contracted meanings.

The diagnostic target state and estimator belief are separate projections:
...
`python-control` is a non-authoritative numerical evaluation adapter. It may replay fixed probe
sequences, simulate a fixed-dimensional projection, and score candidate policies. It does not own
workflow legality, categorical probe selection, evidence admission, or promotion. Legal probes are
derived by the pure kernel and enumerated outside continuous optimization so a solver cannot emit a
fractional blend of actions.

The authoritative root forbids floating-point values. Internal numerical arrays and raw simulation
traces therefore remain non-authoritative artifacts referenced by exact-byte digest. Any admitted
`ControlPolicyEvaluation` uses CUE-defined integer or tagged-decimal projections, names the feature
schema and model artifact digests, binds the evaluated candidate action sequence, and records
approximation/error bounds. The pure kernel independently derives whether that evaluation can rank
an already legal probe; it never accepts a model prediction as evidence that a runtime claim is
true.
