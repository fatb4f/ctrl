...
| CPython runtime providers | Evidence adapters | Emit bounded typed runtime artifacts |
| Pydantic CPython transports | Generated from local CUE | Closed Python evidence boundary |
| python-control | Derived numerical adapter | Replay and score already legal probe policies |
...
| CPython capture integrity | Local mandatory semantic law | CUE plus Python normalization |
| Runtime-value projection | Not expected to be native | Local closed CUE/Pydantic extension |
| Control-policy evaluation | Not expected to be native | Non-authoritative local projection |
...
Model A/B authority
  ...
  → bounded CPython and tool observations
  → optional control-policy replay and ranking
...
CPython runtime evidence enters this seam only as a canonical artifact plus a
rollout-linked `ProbeObservation` provenance envelope. The observation never
authors its own supports/refutes result. CUE admits the bounded artifact and the
pure kernel derives evaluation meaning from the exact obligation, probe,
validator, evidence-role, and rollout closure.

python-control outputs are derived model-evaluation artifacts, not runtime
facts. Legal categorical probes come from the pure kernel; the numerical
adapter may replay or rank only enumerated legal candidates. Raw floating-point
traces remain exact-byte artifacts because the authoritative PPF JSON domain
forbids floats. Any admitted summary uses the local CUE-defined integer or
tagged-decimal `ControlPolicyEvaluation` contract and cannot independently
satisfy an evidence role or authorize promotion.
