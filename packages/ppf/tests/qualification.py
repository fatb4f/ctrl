...
def _subject_digest(paths: Iterable[Path]) -> str:
    def subject_path(path: Path) -> str:
        try:
    ...
    for path in sorted(paths, key=subject_path):
        digest.update(subject_path(path).encode())
        digest.update(b"\0")
...
def _fixture_adapter_probe() -> None:
    result = validate_paths(
        ...
        context=ValidationContext(ROOT, WORKSPACE / "uv.lock"),
    )
    assert result.valid
